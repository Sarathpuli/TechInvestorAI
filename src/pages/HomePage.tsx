// Enhanced HomePage.tsx - Fixed version
...
// CompareStocks Component with improved search
const CompareStocks: React.FC<{ user: User | null }> = ({ user }) => {
  const [selectedStocks, setSelectedStocks] = useState<string[]>(['', '', '', '']);
  const [searchResults, setSearchResults] = useState<CompanySearchResult[][]>([[], [], [], []]);
  const [showResults, setShowResults] = useState<boolean[]>([false, false, false, false]);
  const [loading, setLoading] = useState(false);
  const [comparisonData, setComparisonData] = useState<StockComparison[]>([]);
  const [isComparing, setIsComparing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const searchTimeoutRefs = useRef<(NodeJS.Timeout | null)[]>([null, null, null, null]);

  // Search for companies by name or symbol for specific search bar
  const searchCompanies = useCallback(async (searchTerm: string, index: number) => {
    if (!searchTerm.trim() || searchTerm.length < 2) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.result && Array.isArray(data.result)) {
          ...
          setSearchResults(prev => {
            const updated = [...prev];
            updated[index] = formattedResults;
            return updated;
          });
        }
      }
  ...
  // Debounced search for each input
  const handleSearchInput = (value: string, index: number) => {
    ...
    searchTimeoutRefs.current[index] = setTimeout(() => {
      searchCompanies(value, index);
    }, 300);
  };

  // Handle stock selection
  const handleStockSelect = (symbol: string, name: string, index: number) => {
    setSelectedStocks(prev => {
      const updated = [...prev];
    ...
    // Clear search results for this input
    setSearchResults(prev => {
      const updated = [...prev];
      updated[index] = [];
      return updated;
  ...
  // Fetch comprehensive stock data for comparison
  const fetchStockData = async (symbol: string): Promise<StockComparison | null> => {
    try {
      ...
      const [quote, profile, financials, recommendations] = await Promise.all([
        quoteRes.json(),
        profileRes.json(),
        financialsRes.json(),
        metricsRes.json()
      ...
      const rating = latestRec ? 
        latestRec.strongBuy > latestRec.strongSell ? 'Buy' :
        latestRec.strongSell > latestRec.strongBuy ? 'Sell' : 'Hold' : 'N/A';
      ...
      return {
        ...
        pegRatio: financials?.metric?.pegRatio || 0,
        dividend: financials?.metric?.dividendYieldIndicatedAnnual || 0,
        volume: quote?.v || 0,
        dayHigh: quote?.h || 0,
        dayLow: quote?.l || 0,
        yearHigh: financials?.metric?.['52WeekHigh'] || 0,
        yearLow: financials?.metric?.['52WeekLow'] || 0,
        eps: financials?.metric?.epsBasicExclExtraItemsTTM || 0,
        beta: financials?.metric?.beta || 0,
        revenue: financials?.metric?.revenueTTM || 0,
        grossMargin: financials?.metric?.grossMarginTTM || 0,
        operatingMargin: financials?.metric?.operatingMarginTTM || 0,
        profitMargin: financials?.metric?.netProfitMarginTTM || 0,
        roe: financials?.metric?.roeTTM || 0,
        roa: financials?.metric?.roaTTM || 0,
        debtToEquity: financials?.metric?.totalDebt2TotalEquityQuarterly || 0,
        currentRatio: financials?.metric?.currentRatioQuarterly || 0,
        quickRatio: financials?.metric?.quickRatioQuarterly || 0,
        priceToBook: financials?.metric?.pbQuarterly || 0,
        priceToSales: financials?.metric?.psQuarterly || 0,
        analystRating: rating,
        targetPrice: latestRec?.targetMean || 0
      };
    } catch (error) {
      console.error(`Error fetching data for ${symbol}:`, error);
      return null;
  ...
  const handleCompare = async () => {
    ...
    try {
      const comparisonPromises = validStocks.map(stock => fetchStockData(stock.toUpperCase()));
      const results = await Promise.all(comparisonPromises);
      const validResults = results.filter(result => result !== null) as StockComparison[];
  ...
  const formatNumber = (num: number, isPrice: boolean = false, isCurrency: boolean = false) => {
  ...
  const comparisonMetrics = [
    { key: 'price', label: 'Current Price', format: 'price', higherBetter: false },
    { key: 'marketCap', label: 'Market Cap', format: 'number', higherBetter: true },
    { key: 'peRatio', label: 'P/E Ratio', format: 'ratio', higherBetter: false },
    { key: 'pegRatio', label: 'PEG Ratio', format: 'ratio', higherBetter: false },
    { key: 'priceToBook', label: 'P/B Ratio', format: 'ratio', higherBetter: false },
    { key: 'priceToSales', label: 'P/S Ratio', format: 'ratio', higherBetter: false },
    { key: 'eps', label: 'EPS (TTM)', format: 'currency', higherBetter: true },
    { key: 'dividend', label: 'Dividend Yield', format: 'percent', higherBetter: true },
    { key: 'beta', label: 'Beta', format: 'ratio', higherBetter: false },
    { key: 'roe', label: 'ROE (%)', format: 'percent', higherBetter: true },
    { key: 'roa', label: 'ROA (%)', format: 'percent', higherBetter: true },
    { key: 'debtToEquity', label: 'Debt/Equity', format: 'ratio', higherBetter: false },
    { key: 'currentRatio', label: 'Current Ratio', format: 'ratio', higherBetter: true },
    ...
    { key: 'profitMargin', label: 'Profit Margin (%)', format: 'percent', higherBetter: true }
  ];
            ...
            {selectedStocks.map((stock, index) => (
              <div key={index} className="relative">
                <label className="block text-sm text-gray-300 mb-2">
                  Stock {index + 1} {index < 2 && <span className="text-red-400">*</span>}
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    value={stock}
                    onChange={(e) => handleSearchInput(e.target.value, index)}
                    onFocus={() => {
                      if (stock) {
                        setShowResults(prev => {
                          const updated = [...prev];
                    ...
                    className="w-full pl-10 pr-4 py-3 rounded-lg bg-gray-700 border border-gray-600 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
                  />
                </div>
                ...
                {showResults[index] && searchResults[index].length > 0 && (
                  <div className="absolute z-10 w-full mt-1 bg-gray-800 border border-gray-600 rounded-lg shadow-xl max-h-48 overflow-y-auto">
                    {searchResults[index].map((result, resultIndex) => (
                      <button
                        key={resultIndex}
                        onClick={() => handleStockSelect(result.symbol, result.name, index)}
                        className="w-full px-4 py-3 text-left hover:bg-gray-700 transition-colors border-b border-gray-700 last:border-b-0"
                      >
                        <div className="flex justify-between items-center">
                          <div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <button
                ...
                disabled={loading || selectedStocks.filter(s => s.trim()).length < 2}
                className="bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed px-6 py-3 rounded-lg flex items-center space-x-2 transition-colors font-medium"
              >
                {loading ? (
                ...
                <span>{loading ? 'Comparing...' : 'Compare Stocks'}</span>
              </button>
            ...
            {user && (
              <div className="text-sm text-blue-400 bg-blue-900/20 px-3 py-2 rounded-lg">
                💡 Pro Tip: Compare fundamentals to make informed decisions
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {comparisonData.map((stock) => (
                <div key={stock.symbol} className="bg-gray-700/50 p-4 rounded-lg border border-gray-600">
                  <div className="text-center">
                    <h4 className="font-bold text-lg text-white mb-1">{stock.symbol}</h4>
                    <p className="text-sm text-gray-300 mb-3 truncate">{stock.name}</p>
                    <p className="text-2xl font-bold text-green-400 mb-2">{formatNumber(stock.price, true)}</p>
                    <div className="space-y-1 text-xs">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-white">{stock.peRatio ? stock.peRatio.toFixed(2) : 'N/A'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Rating:</span>
                        <span className={`font-medium ${
                          stock.analystRating === 'Buy' ? 'text-green-400' :
                          stock.analystRating === 'Sell' ? 'text-red-400' : 'text-yellow-400'
                        }`}>
                          {stock.analystRating}
                        </span>
                      </div>
            ...
            <div className="bg-gray-700/30 rounded-lg p-4 overflow-x-auto">
              <h4 className="font-semibold text-white mb-4">Financial Metrics Comparison</h4>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-600">
                      ...
                      {comparisonData.map(stock => (
                        <th key={stock.symbol} className="text-center py-3 px-4 text-white font-medium">
                          {stock.symbol}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {comparisonMetrics.map((metric, index) => (
                      <tr key={metric.key} className={`border-b border-gray-700/50 ${index % 2 === 0 ? 'bg-gray-800/30' : ''}`}>
                        <td className="py-3 px-4 text-gray-300 font-medium">{metric.label}</td>
                        {comparisonData.map(stock => {
                          const value = stock[metric.key as keyof StockComparison] as number;
                          ...
                          if (value !== 0 && value !== null && value !== undefined) {
                            switch (metric.format) {
                              case 'price':
                                ...
                                break;
                              case 'currency':
                                formattedValue = formatNumber(value, false, true);
                                break;
                              case 'percent':
                          ...
                          return (
                            <td key={stock.symbol} className="py-3 px-4 text-center text-white">
                              {formattedValue}
                            </td>
                          );
                        })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gray-700/30 rounded-lg p-4">
                <h4 className="font-semibold text-white mb-4">52-Week Range</h4>
                {comparisonData.map(stock => (
                  <div key={stock.symbol} className="mb-4 last:mb-0">
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-medium text-white">{stock.symbol}</span>
                      <span className="text-sm text-gray-400">
                        {formatNumber(stock.yearLow, true)} - {formatNumber(stock.yearHigh, true)}
                      </span>
                    </div>
                    <div className="w-full bg-gray-600 rounded-full h-2">
                      <div 
                      ...
                      >
                        <div 
                          className="absolute top-0 w-2 h-2 bg-white rounded-full transform -translate-y-0"
                          style={{ 
              ...
              <div className="bg-gray-700/30 rounded-lg p-4">
                <h4 className="font-semibold text-white mb-4">Analyst Targets</h4>
                {comparisonData.map(stock => (
                  <div key={stock.symbol} className="flex justify-between items-center py-2 border-b border-gray-700 last:border-b-0">
                    <div>
                      <span className="font-medium text-white">{stock.symbol}</span>
                      <span className={`ml-2 text-sm px-2 py-1 rounded ${
                        stock.analystRating === 'Buy' ? 'bg-green-900 text-green-300' :
                        stock.analystRating === 'Sell' ? 'bg-red-900 text-red-300' : 'bg-yellow-900 text-yellow-300'
                      }`}>
                        {stock.analystRating}
                      </span>
                    </div>
                    <div className="text-right">
                      <p className="text-white font-medium">{formatNumber(stock.targetPrice, true)}</p>
                      <p className={`text-sm ${
                        stock.targetPrice > stock.price ? 'text-green-400' : 
                        stock.targetPrice < stock.price ? 'text-red-400' : 'text-gray-400'
                      }`}>
                        {stock.targetPrice > 0 ? 
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <main className="p-6 max-w-7xl mx-auto">
        ...
        {/* Compare Stocks Section - Only for logged in users */}
        {user ? (
          <section className="mb-8">
            <CompareStocks user={user} />
          </section>
        ) : (
          <section className="mb-8">
            <LoginRequired 
              title="Compare Stocks" 
              description="Please login to compare stocks side by side and analyze their financial metrics"
              icon={BarChart3}
